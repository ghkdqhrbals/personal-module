...
class ArxivXmlParser {
    ...
    private fun entryToPaper(entry: SyndEntry): ArxivPaper {
        ...
        val url = entry.link ?: entry.links?.firstOrNull()?.href ?: throw CommonException("Entry has no URL")
    ...
    private fun extractTotalResults(xml: String): Int? {
        val regex =
            Regex("<opensearch:totalResults>\\s*([0-9]+)\\s*</opensearch:totalResults>", RegexOption.IGNORE_CASE)
...
data class ArxivPaper(
    ...
    fun toPaperEntity(): PaperEntity {
        return PaperEntity(
            ...
            publishedAt = publishedDate?.let { OffsetDateTime.parse(it) }
                ?: throw CommonException("Published date is null"),
            searchDate = OffsetDateTime.now(),
            url = url,
            abstract = abstract ?: "",
