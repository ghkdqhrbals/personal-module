...
* member는 직접 shard owner를 최종 결정하지 않는다. coordinator가 내려준 target assignment와 revoke/assign 명령에 수렴한다.
...
* 외부 side effect 중복 방지는 idempotency key와 processing marker가 담당한다.

## 문서 작성 규칙

* "정확히 한 번" 같은 표현은 쓰지 않는다. 보장 범위와 실패 경계를 명시한다.
* epoch, lease token, heartbeat TTL, rebalance timeout, metadata version 같은 fencing 조건을 숨기지 않는다.
