...
def test_ask_stream_rate_limit_falls_back_to_request_client_host(monkeypatch):
    monkeypatch.setattr(guestbook_app, "ASK_RATE_LIMIT_PER_MINUTE", 1)
    monkeypatch.setattr(guestbook_app, "_public_mcp_server_url", lambda request: "https://lowfidev.cloud/mcp/")
    monkeypatch.setattr(
        guestbook_app,
    ...
    first = client.post("/ask/stream", json=_ask_payload("first"))
    second = client.post("/ask/stream", json=_ask_payload("second"))

    assert first.status_code == 200
    assert second.status_code == 429
    assert "testclient" in guestbook_app._ask_rate_limit_hits
