...
class OAuthClientFactoryTest {
    ...
    private fun registryWithAll(kind: OauthProviderKind) =
        OauthProviderRegistry.build {
            add(kind) {
                ...
                codePath = "https://auth.example.com/authorize"
        ...
        OauthProviderRegistry.build {
            add(kind) {
                ...
                when (missing) {
                    ...
                    else -> codePath = "https://auth.example.com/authorize"
                }
                when (missing) {
    ...
    private class StubService(kind: OauthProviderKind) : OAuthService(kind) {
        override fun buildAuthorizationUrl(state: String?): String = "stub"
        override fun providerKind(): OauthProviderKind = kind
        override fun fetchUserInfo(accessToken: String): OAuthUserInfo =
            OAuthUserInfo(provider = kind, providerId = "id", rawAttributes = emptyMap())
    ...
    inner class Success {
        ...
        fun returns_service_when_config_complete() {
            ...
            val factory = OAuthClientFactory(listOf(StubService(kind)), registryWithAll(kind))
            val svc = assertDoesNotThrow<OAuthService> { factory.get(kind.name) }
            assertSame(kind, svc.kind)
        }
    }
    ...
    inner class MissingConfig {
        ...
        fun missing_client_id() {
            ...
            val factory = OAuthClientFactory(listOf(StubService(kind)), registryMissing(kind, "clientId"))
            assertThrows(IllegalStateException::class.java) { factory.get(kind.name) }
        }

        @Test
        @DisplayName("redirectUri 누락 시 예외")
        fun missing_redirect_uri() {
            val kind = OauthProviderKind.GOOGLE
            val factory = OAuthClientFactory(listOf(StubService(kind)), registryMissing(kind, "redirectUri"))
            assertThrows(IllegalStateException::class.java) { factory.get(kind.name) }
        }

        @Test
        @DisplayName("codePath 누락 시 예외")
        fun missing_code_path() {
            val kind = OauthProviderKind.NAVER
            val factory = OAuthClientFactory(listOf(StubService(kind)), registryMissing(kind, "codePath"))
            assertThrows(IllegalStateException::class.java) { factory.get(kind.name) }
        }

        @Test
        @DisplayName("tokenPath 누락 시 예외")
        fun missing_token_path() {
            val kind = OauthProviderKind.NAVER
            val factory = OAuthClientFactory(listOf(StubService(kind)), registryMissing(kind, "tokenPath"))
            assertThrows(IllegalStateException::class.java) { factory.get(kind.name) }
        }

        @Test
        @DisplayName("userInfoPath 누락 시 예외")
        fun missing_userinfo_path() {
            val kind = OauthProviderKind.KAKAO
            val factory = OAuthClientFactory(listOf(StubService(kind)), registryMissing(kind, "userInfoPath"))
            assertThrows(IllegalStateException::class.java) { factory.get(kind.name) }
        }
    }
