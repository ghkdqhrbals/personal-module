...
import org.ghkdqhrbals.client.paper.ArxivHttpClient
import org.ghkdqhrbals.client.paper.ArxivXmlParser
import org.ghkdqhrbals.client.paper.service.PaperService
...
class PaperSearchAndStoreStreamListener(
    ...
    private fun searchAndStore(event: PaperSearchAndStoreEvent): List<org.ghkdqhrbals.client.paper.ArxivPaper> {
        // 1. arXiv API 호출
        val url = buildRequestUrl(event.query, event.categories, event.maxResults, event.page)
        val xmlBytes = httpClient.fetchXml(url) ?: return emptyList()

        // 2. XML 파싱
        val parseResult = xmlParser.parse(
            xmlBytes,
        ...
        logger().info("[PaperSearchAndStoreListener] Found ${parseResult.papers.size} papers from arXiv")

        val entities = parseResult.papers.map { it.toPaperEntity() }
        val savedEntities = paperService.upsertPapersAndGetUnsummarized(entities)
        val needToSummarize = parseResult.papers.filter { paper ->
