...
private fun Subscribe.toPaperSearchAndStoreEvent(page: PageRequest): PaperSearchAndStoreEvent {
    ...
    val query = when (this.subscribeType) {
        org.ghkdqhrbals.client.domain.subscribe.entity.SubscribeType.CATEGORY -> {
            "cat:${this.name}"
        }
        org.ghkdqhrbals.client.domain.subscribe.entity.SubscribeType.KEYWORD -> {
            "all:${this.name}"
        }
        org.ghkdqhrbals.client.domain.subscribe.entity.SubscribeType.AUTHOR -> {
            "au:${this.name}"
        }
        else -> {
            this.name
